...
SabertoothPropulsion::SabertoothPropulsion(ros::NodeHandle node_handle, ros::NodeHandle private_node_handle)
{
	...
	uint8_t timeout_packet[4];
	sabertooth_propulsion_driver_->sabertooth_motor_driver_->setSerialTimeout(10, &timeout_packet[0]);
	sabertooth_serial_->writePacket(&timeout_packet[0]);
...
{	
	if(sabertooth_serial_->isOpen() && actuation_watchdog_->isActive())
	{
		...
		ROS_INFO("left packet %i %i %i %i", left_packet[0], left_packet[1], left_packet[2], left_packet[3]);
...
void SabertoothPropulsion::callbackPropulsionRight(const std_msgs::Float32::ConstPtr& right_message)
{
	if(sabertooth_serial_->isOpen() && actuation_watchdog_->isActive())
	{
		...
		ROS_INFO("right packet %i %i %i %i", right_packet[0], right_packet[1], right_packet[2], right_packet[3]);
